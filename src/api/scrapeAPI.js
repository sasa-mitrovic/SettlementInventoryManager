...
export async function handleScrapeRequest(req, res) {
  try {
    ...
    if (!force && lastScrape && Date.now() - lastScrape < SCRAPE_COOLDOWN) {
      const remainingCooldown = Math.ceil((SCRAPE_COOLDOWN - (Date.now() - lastScrape)) / 1000);
    ...
    console.log(`[ScrapeAPI] Starting scrape for user ${userId}, settlement ${settlementId}`);
    ...
    if (result.success) {
      ...
      console.log(`[ScrapeAPI] Scrape completed: ${result.itemCount} items for settlement ${settlementId}`);
...
export function setupScrapeAPI(app) {
  ...
  app.get('/api/scrape-status/:settlementId/:userId', (req, res) => {
    ...
    const status = {
      ...
      cooldownRemaining: lastScrape ? Math.max(0, SCRAPE_COOLDOWN - (Date.now() - lastScrape)) : 0
    };
